...
class LossManager:
    ...
    def ssim(self,
        ...
        # SSIM luminance
        mean1_bhw3 = reducer(img1_bhw3, filter_size, filter_sigma)
        mean2_bhw3 = reducer(img2_bhw3, filter_size, filter_sigma)
    ...
    def warp_loss(self,
        ...
        valid_mask_bhw = pixel_src_bhw2[..., 0] >= 0 & \
                         pixel_src_bhw2[..., 0] <= w - 1 & \
                         pixel_src_bhw2[..., 1] >= 0 & \
                         pixel_src_bhw2[..., 1] <= h - 1
        ...
        img_loss = 0.15 * tf.abs(img_diff_bhw3) + .85 * (1 - img_ssim_bhw3) / 2.
        img_loss = tf.reduce_sum(img_loss * valid_mask_bhw1)
        img_loss = tf.math.divide_no_nan(img_loss, tf.reduce_sum(valid_mask_bhw1) * 3)

        # geometric loss
        depth_diff_bhw1 = tf.abs(depth_computed_bhw1 - depth_proj_bhw3)
        geo_loss = depth_diff_bhw1 / (depth_computed_bhw1 + depth_proj_bhw3)
    ...
    def all_loss(self,
        img1_bhw3: tf.Tensor,
        img2_bhw3: tf.Tensor,
        depth1_bhw3: tf.Tensor,
        depth2_bhw3: tf.Tensor,
        c1_T_c2: Pose3D,
        c2_T_c1: Pose3D,
    ):
        smooth_loss1 = self.smooth_loss(depth1_bhw3, img1_bhw3)
        smooth_loss2 = self.smooth_loss(depth2_bhw3, img2_bhw3)

        img_loss1, geo_loss1 = self.warp_loss(img2_bhw3, depth2_bhw3, img1_bhw3, depth1_bhw3, c1_T_c2)
        img_loss2, geo_loss2 = self.warp_loss(img1_bhw3, depth1_bhw3, img2_bhw3, depth2_bhw3, c2_T_c1)
